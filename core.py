...
class NanonisController(nanonis_programming_interface):
    ...
    def SignalIndexGet(self, name):
        ...
        if name not in [n[1] for n in self.signal_chart]:
            raise nanonisException('Invalid signal name.')
        return next(index for index, n in self.signal_chart if n == name)
    
    def SignalInSlotSet(self, Slot, RT_signal_index):
        # Slot (int) is the index of the slot in the Signals Manager, between 0 and 23
        # RT_signal_index (int) is the index of the real-time signal assigned to the slot, between 0 and 127
        self.send('Signals.InSlotSet', 'int', Slot, 'int', RT_signal_index)

    def SignalInSlotGet(self, name):
        # Get the information of signals and slots
        response = self.send('Signals.InSlotsGet')
        cursor = 0

        # Read the number of signal names and the list of signal names
        ...
        # Read the size and list of signal indexes
        Signals_indexes_size = from_binary('int', response['body'][cursor: cursor + 4])
        cursor += 4
        Signals_indexes_number = Signals_indexes_size // 4  # Calculate the number of signal indexes
        Signals_indexes = {}

        # Process the signal indexes and associate them with signal names
        for i in range(Signals_indexes_size):
            signal_index = from_binary('int', response['body'][cursor: cursor + 4])
            Signals_indexes[signal_index] = Signals_names[i]
            cursor += 4

        # Find the Signal Index corresponding to the given signal name
        if name in Signals_names:
            signal_index = Signals_names.index(name)  # Get the index of the signal name
            return signal_index  # Return the Signal Index
        else:
            raise ValueError(f"Signal '{name}' not found in slots.")
    ...
    def ScanBufferSet(self, Pixels, Lines, Channel_indexes): 
        """
        Set the configuration of the scanning buffer.
        
        Users can define the number of pixels per line, the number of scan lines, and the allocation of signal channels using this method to accommodate different scanning requirements.
        
        Parameters:
            Pixels (int): Number of pixels per line.
            Lines (int): Number of scan lines.
            Channel_indexes (list of int): List of signal indices.
        
        Exceptions:
            ValueError: Raised if the length of Channel_indexes is not 2 or 3. This exception handles the mismatch between decoded numbers and positions when testing demo mode vs real (actual scan images) on Nanonis devices.
            This part of the code can be modified according to actual conditions.
        """
        if len(Channel_indexes) == 2:
            ...
            raise ValueError("Channel_indexes must have length 2 or 3.")
    ...
    TipShaper by Zhiwen Zhu at 2024/03/04 16:57
    ...
    def AdjustTipToPiezoCenter(self,threshold = 0.7):
        ...
        if Z_pos > Z_lim['high'] * threshold or Z_pos < Z_lim['low'] * threshold:
            ...
            if Z_pos > Z_lim['high']*threshold:                     # the tip is too close to the surface, so move the tip Z-
                ...
                Tip_move_direction = 'Z+'                 
            target_low = Z_lim['low'] * 0.2
            target_high = Z_lim['high'] * 0.2
            ...
            max_adjust_loops = 12
            adjust_loops = 0
            while Z_pos_after < target_low or Z_pos_after > target_high:
                adjust_loops += 1
                if adjust_loops > max_adjust_loops:
                    logging.warning('AdjustTipToPiezoCenter reached max loops; stop auto-approach retry to avoid infinite cycle.')
                    break
